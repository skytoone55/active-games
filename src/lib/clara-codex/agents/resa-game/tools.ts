...
export const checkGameAvailability = tool({
  ...
  inputSchema: z.object({
    ...
    numberOfGames: z.number().int().min(1).max(6),
  }),
  execute: async (params) => {
    ...
    const result = await checkGameAvailabilityFn({
      ...
      participants: params.participants, type: 'GAME',
      gameArea: params.gameArea, numberOfGames: params.numberOfGames,
    })

    if (result.available) {
      return { available: true, message: 'Slot is available.', details: { gameArea: params.gameArea, numberOfGames: params.numberOfGames } }
    }

    // Build alternatives
    const alternatives: Array<{ id: string; label: string; date: string | null; time: string | null; type: string }> = []

    const before = await findNearestSlot(params, 'before')
    if (before) alternatives.push({ id: `before_${before.replace(':', '')}`, label: `Before: ${before}`, date: params.date, time: before, type: 'slot' })

    const after = await findNearestSlot(params, 'after')
...
export function createGameBookingLink(context: AgentContext) {
  return tool({
    ...
    execute: async (params) => {
      ...
      urlParams.set('games', String(params.numberOfGames))
      ...
      try {
        await supabase.from('orders').insert({
          ...
          number_of_games: params.numberOfGames,

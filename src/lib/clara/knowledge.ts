 ...
 * Infos statiques pour réponses rapides sans appels DB
 *
 * IMPORTANT: Ces infos viennent du site, des CGV et du CRM.
 * Les prix EVENT sont dynamiques (voir formules dans icount_event_formulas).
 */

export const CLARA_KNOWLEDGE = `
...
**IMPORTANT:** Sur demande, on peut ouvrir en dehors des horaires (le matin ou plus tard le soir), tous les jours. Toujours proposer cette option si le client a des contraintes horaires.
...
**Exemple:** 20 participants veulent Laser + Active
→ Chercher formule BOTH avec 20 dans [min_participants, max_participants]
→ Si price_per_person = 110₪, total = 20 × 110 = 2200₪
→ + salle si réservée
...
/**
 * Résumé court pour le prompt système
 */
export function getKnowledgeSummary(): string {
  return CLARA_KNOWLEDGE
}

...
import { FileX, Loader2, AlertTriangle, ChevronDown, ChevronUp, RotateCcw, Trash2, CheckCircle } from 'lucide-react'
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      >
                        ...
                        {t('admin.out_orders.mark_in')}
                      </button>
                    </div>

                    <div className={`grid grid-cols-2 md:grid-cols-4 gap-2 text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                      {order.booking && (
                        <>
                          <div>
                            <span className="font-medium">{t('admin.out_orders.date')}:</span> {new Date(order.booking.start_datetime).toLocaleDateString()}
                          </div>
                          <div>
                            <span className="font-medium">{t('admin.out_orders.players')}:</span> {order.booking.participants_count}
                          </div>
                        </>
                      )}
                      {order.number_of_games && (
                        <div>
                          <span className="font-medium">{t('admin.out_orders.games')}:</span> {order.number_of_games}
                        </div>
                      )}
                      <div>
                        <span className="font-medium">{t('admin.out_orders.phone')}:</span> {order.customer_phone}
                      </div>
                    </div>
                  </div>
                </div>

...
function sectionDescription(section: SectionId): string {
  ...
  if (section === 'branches') return 'Branches where Clara is allowed to operate.'
  ...
  return 'Exact list of backend tools available to Clara.'
}
...
export function GeneralTab({ isDark }: GeneralTabProps) {
  ...
  function validateSection(section: SavableSectionId): string | null {
    if (section === 'overview') {
      if (isActive && !settings.primary_prompt.trim()) return 'Primary prompt is required before activating.'
      if (isActive && selectedBranchCount === 0) return 'At least one active branch is required before activation.'
      return null
    }
    if (section === 'model') {
      if (models.length === 0) return 'No validated LLM key found.'
      if (settings.temperature < 0 || settings.temperature > 1) return 'Temperature must stay between 0 and 1.'
      if (!Number.isFinite(settings.max_tokens) || settings.max_tokens < 256 || settings.max_tokens > 8192) return 'Max tokens must stay between 256 and 8192.'
      if (!Number.isFinite(settings.auto_resume_minutes) || settings.auto_resume_minutes < 1 || settings.auto_resume_minutes > 180) return 'Auto resume must stay between 1 and 180 minutes.'
      if (!Number.isFinite(settings.auto_activate_delay_minutes) || settings.auto_activate_delay_minutes < 0 || settings.auto_activate_delay_minutes > 60) return 'Auto activate delay must stay between 0 and 60 minutes.'
      return null
    }
    if (section === 'branches') {
      if (isActive && selectedBranchCount === 0) return 'Select at least one branch before activation.'
      return null
    }
    if (section === 'schedule') {
      ...
      for (const day of DAYS) {
        ...
        if (!isValidTimeValue(row.start) || !isValidTimeValue(row.end)) return `Invalid time format for ${day.label}.`
        if (row.start >= row.end) return `${day.label}: start time must be before end time.`
      }
      return null
    }
    if (section === 'prompt') {
      if (!settings.primary_prompt.trim()) return 'Primary prompt cannot be empty.'
      if (primaryPromptChanged && !primaryPromptUnlocked) return 'To edit and save primary prompt, check the safety checkbox first.'
      return null
    }
    // fallbacks
    ...
    for (const key of keys) {
      for (const locale of ['fr', 'en', 'he'] as const) {
        ...
        if (!value || !value.trim()) return `${key} (${locale}) cannot be empty.`
      }
    }
    return null
  }
  ...
  async function saveSection(section: SavableSectionId) {
    try {
      ...
      if (validationError) { setSectionMessage(section, 'error', validationError); return }
      setSavingSection(section)
      const payload = { id: settingsId, is_active: isActive, settings }
      const response = await fetch('/api/admin/clara-codex/settings', {
        method: 'PUT',
      ...
      if (section === 'prompt') setOriginalPrimaryPrompt(settings.primary_prompt || '')
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <Icon className={classNames('w-4 h-4', isDark ? 'text-orange-400' : 'text-orange-600')} />
                <div>
            ...
            {open && (
              <div className="px-5 pb-5 space-y-4">
                  ...
                  <>
                    <div className="grid md:grid-cols-3 gap-4">
                      <label className="space-y-1">
                        ...
                        <select value={settings.model} disabled={models.length === 0} onChange={(e) => { setSettings(prev => ({ ...prev, model: e.target.value })); clearSectionStatus('model') }} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')}>
                          {Object.entries(groupedModels).map(([provider, providerModels]) => (
                            <optgroup key={provider} label={provider}>{providerModels.map(model => (<option key={model.id} value={model.id}>{model.name}</option>))}</optgroup>
                          ))}
                        </select>
                      </label>
                      <label className="space-y-1">
                        <span className={classNames('text-sm', isDark ? 'text-gray-300' : 'text-gray-700')}>Temperature</span>
                        <input type="number" min={0} max={1} step={0.1} value={settings.temperature} onChange={(e) => { setSettings(prev => ({ ...prev, temperature: Number(e.target.value) })); clearSectionStatus('model') }} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                      </label>
                      <label className="space-y-1">
                        <span className={classNames('text-sm', isDark ? 'text-gray-300' : 'text-gray-700')}>Max Tokens</span>
                        <input type="number" min={256} max={8192} step={128} value={settings.max_tokens} onChange={(e) => { setSettings(prev => ({ ...prev, max_tokens: Number(e.target.value) })); clearSectionStatus('model') }} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                      </label>
                    </div>
                    <div className="grid md:grid-cols-3 gap-4">
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input type="checkbox" checked={settings.faq_enabled} onChange={(e) => { setSettings(prev => ({ ...prev, faq_enabled: e.target.checked })); clearSectionStatus('model') }} />
                        <span className={isDark ? 'text-gray-200' : 'text-gray-800'}>Use FAQ as truth source</span>
                      </label>
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input type="checkbox" checked={settings.enforce_email_for_link} onChange={(e) => { setSettings(prev => ({ ...prev, enforce_email_for_link: e.target.checked })); clearSectionStatus('model') }} />
                        <span className={isDark ? 'text-gray-200' : 'text-gray-800'}>Require email before booking link</span>
                      </label>
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input type="checkbox" checked={settings.silent_human_call} onChange={(e) => { setSettings(prev => ({ ...prev, silent_human_call: e.target.checked })); clearSectionStatus('model') }} />
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      <label className="space-y-1">
                        ...
                        <input type="number" min={1} max={180} value={settings.auto_resume_minutes} onChange={(e) => { setSettings(prev => ({ ...prev, auto_resume_minutes: Number(e.target.value) })); clearSectionStatus('model') }} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                      </label>
                      <label className="space-y-1">
                        <span className={classNames('text-sm', isDark ? 'text-gray-300' : 'text-gray-700')}>Auto activate delay (minutes)</span>
                        <input type="number" min={0} max={60} value={settings.auto_activate_delay_minutes} onChange={(e) => { setSettings(prev => ({ ...prev, auto_activate_delay_minutes: Number(e.target.value) })); clearSectionStatus('model') }} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                      </label>
                    </div>
                      ...
                      {branches.map(branch => {
                        ...
                        return (
                          <label key={branch.id} className="inline-flex items-center gap-2 text-sm">
                            <input type="checkbox" checked={checked} onChange={(e) => { setSettings(prev => ({ ...prev, active_branches: e.target.checked ? [...prev.active_branches, branch.id] : prev.active_branches.filter(id => id !== branch.id) })); clearSectionStatus('branches') }} />
                ...
                {section === 'schedule' && (
                  <>
                    <label className="inline-flex items-center gap-2 text-sm">
                      <input type="checkbox" checked={settings.schedule_enabled} onChange={(e) => { setSettings(prev => ({ ...prev, schedule_enabled: e.target.checked })); clearSectionStatus('schedule') }} />
                    ...
                    {DAYS.map(day => (
                      <div key={day.key} className="grid grid-cols-4 gap-3 items-center">
                        <label className="inline-flex items-center gap-2 text-sm">
                          <input type="checkbox" checked={settings.schedule[day.key].enabled} onChange={(e) => { setSettings(prev => ({ ...prev, schedule: { ...prev.schedule, [day.key]: { ...prev.schedule[day.key], enabled: e.target.checked } } })); clearSectionStatus('schedule') }} />
                          <span className={isDark ? 'text-gray-200' : 'text-gray-800'}>{day.label}</span>
                        </label>
                        <input type="time" value={settings.schedule[day.key].start} onChange={(e) => { setSettings(prev => ({ ...prev, schedule: { ...prev.schedule, [day.key]: { ...prev.schedule[day.key], start: e.target.value } } })); clearSectionStatus('schedule') }} className={classNames('rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                        <input type="time" value={settings.schedule[day.key].end} onChange={(e) => { setSettings(prev => ({ ...prev, schedule: { ...prev.schedule, [day.key]: { ...prev.schedule[day.key], end: e.target.value } } })); clearSectionStatus('schedule') }} className={classNames('rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                      </div>
                    ))}
                ...
                {section === 'prompt' && (
                  <>
                    <div className={classNames('rounded-lg border p-3 space-y-2 text-sm', isDark ? 'bg-gray-900/40 border-gray-700 text-gray-300' : 'bg-gray-50 border-gray-200 text-gray-700')}>
                    ...
                    <div className={classNames('rounded-lg border p-3 space-y-3', isDark ? 'bg-gray-900/40 border-gray-700' : 'bg-gray-50 border-gray-200')}>
                      <p className={classNames('text-sm font-medium', isDark ? 'text-gray-200' : 'text-gray-800')}>Safety confirmation before editing primary prompt</p>
                      <label className="inline-flex items-center gap-2 text-sm">
                        <input type="checkbox" checked={unlockPrimaryPromptChecked} onChange={(e) => { setUnlockPrimaryPromptChecked(e.target.checked); clearSectionStatus('prompt') }} />
                        <span className={isDark ? 'text-gray-300' : 'text-gray-700'}>I understand this prompt controls Clara core behavior.</span>
                      </label>
                    </div>
                    <label className="space-y-1 block">
                      <span className={classNames('text-sm font-medium', isDark ? 'text-gray-300' : 'text-gray-700')}>Primary prompt</span>
                      <textarea value={settings.primary_prompt} onChange={(e) => { setSettings(prev => ({ ...prev, primary_prompt: e.target.value })); clearSectionStatus('prompt') }} rows={16} disabled={!primaryPromptUnlocked} placeholder="Write the full Clara system prompt here." className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900', !primaryPromptUnlocked && 'opacity-60 cursor-not-allowed')} />
                    </label>
                    <label className="space-y-1 block">
                      <span className={classNames('text-sm', isDark ? 'text-gray-300' : 'text-gray-700')}>Event keywords (comma separated)</span>
                      <input value={settings.event_keywords.join(', ')} onChange={(e) => { setSettings(prev => ({ ...prev, event_keywords: e.target.value.split(',').map(x => x.trim()).filter(Boolean) })); clearSectionStatus('prompt') }} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                    </label>
                    <label className="space-y-1 block">
                      <span className={classNames('text-sm', isDark ? 'text-gray-300' : 'text-gray-700')}>Additional directive (optional)</span>
                      <textarea value={settings.custom_prompt} onChange={(e) => { setSettings(prev => ({ ...prev, custom_prompt: e.target.value })); clearSectionStatus('prompt') }} rows={7} className={classNames('w-full rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                    </label>
                      ...
                      <div key={key} className="space-y-2">
                        ...
                        <div className="grid md:grid-cols-3 gap-3">
                          {(['fr', 'en', 'he'] as const).map((locale) => (
                            <textarea key={`${key}-${locale}`} value={settings[key][locale]} onChange={(e) => { setSettings(prev => ({ ...prev, [key]: { ...prev[key], [locale]: e.target.value } })); clearSectionStatus('fallbacks') }} rows={3} placeholder={`${key} (${locale})`} className={classNames('rounded-lg border px-3 py-2 text-sm', isDark ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900')} />
                          ))}
                        </div>
                      </div>
                    ))}
                ...
                {section === 'tools' && (
                  <div className="space-y-3">
                    {CODEX_WHATSAPP_TOOL_CATALOG.map(tool => (
                      <div key={tool.id} className={classNames('rounded-lg border p-3', isDark ? 'bg-gray-900/40 border-gray-700' : 'bg-gray-50 border-gray-200')}>
                        <div className="flex items-center justify-between gap-3">
                          <p className={classNames('font-medium text-sm', isDark ? 'text-gray-200' : 'text-gray-900')}>{tool.label}</p>
                          <code className={classNames('text-xs', isDark ? 'text-blue-300' : 'text-blue-700')}>{tool.id}</code>
                        </div>
                        <p className={classNames('text-sm mt-1', isDark ? 'text-gray-300' : 'text-gray-700')}>{tool.purpose}</p>
                        <p className={classNames('text-xs mt-1', isDark ? 'text-gray-400' : 'text-gray-600')}>Trigger: {tool.trigger}</p>
                      </div>
                    ))}
                  </div>
                )}

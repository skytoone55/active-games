...
export default function AdminPage() {
  ...
  // Wrapper STABLE pour openBookingModal — passé à <AgendaGrid> (mémoïsé).
  // Appelé uniquement au clic : lit toujours la dernière version via ref, donc
  // la grille n'est jamais re-rendue à cause de ce handler (gain de fluidité)
  // sans aucun risque de closure périmée.
  const openBookingModalRef = useRef(openBookingModal)
  openBookingModalRef.current = openBookingModal
  const stableOpenBookingModal = useCallback(
    (hour?: number, minute?: number, booking?: BookingWithSlots, defaultType?: 'GAME' | 'EVENT', defaultGameArea?: 'ACTIVE' | 'LASER') =>
      openBookingModalRef.current(hour, minute, booking, defaultType, defaultGameArea),
    []
  )
  ...
  const getSegmentForCellRooms = useCallback((hour: number, minute: number, roomIndex: number): UISegment | null => {
    ...
    return null
  }, [selectedDate, branches, selectedBranchId, bookings])
      ...
      <main className="p-6">
        ...
        <div className={`rounded-xl overflow-hidden relative ${isDark ? 'bg-gray-800' : 'bg-white'}`} style={{ 
          ...
          <AgendaGrid
            ...
            TOTAL_LASER_ROOMS={TOTAL_LASER_ROOMS}
            TOTAL_ROOMS={TOTAL_ROOMS}
            timeSlots={timeSlots}
            rowHeight={rowHeight}
            selectedDate={selectedDate}
            SLOT_DURATION={SLOT_DURATION}
            obByTimeKey={obByTimeKey}
            laserRooms={laserRooms}
            branchRooms={branchRooms}
            displayTextSize={displayTextSize}
            displayTextWeight={displayTextWeight}
            dateString={dateString}
            segmentIndexActive={segmentIndexActive}
            segmentIndexLaser={segmentIndexLaser}
            getSegmentForCellRooms={getSegmentForCellRooms}
            openBookingModal={stableOpenBookingModal}

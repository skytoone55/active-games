// Active Games Widget for Laser City
(function() {
    'use strict';

    // Check if widget was previously closed
    const wasClosed = localStorage.getItem('activeGamesWidgetClosed');
    if (wasClosed === 'true') {
        // Don't show if closed in last 24 hours
        const closedTime = localStorage.getItem('activeGamesWidgetClosedTime');
    ...
    window.closeActiveGamesWidget = function() {
        ...
        localStorage.setItem('activeGamesWidgetClosed', 'true');
        localStorage.setItem('activeGamesWidgetClosedTime', Date.now().toString());
    };
